...
def demo_service_discovery():
    ...
    # Discover a specific service and call a random instance
    import random
    service_to_discover = "user-service"  # Change as needed
    print(f"\nDiscovering instances of '{service_to_discover}'...")
    try:
        response = requests.get(f"{registry_url}/discover/{service_to_discover}")
        if response.status_code == 200:
            data = response.json()
            instances = data.get('instances', [])
            if not instances:
                print(f"No instances of '{service_to_discover}' found.")
                return
            print(f"Found {len(instances)} instance(s):")
            for i, inst in enumerate(instances):
                print(f"  {i+1}. {inst['address']} (uptime: {inst['uptime_seconds']:.1f}s)")
            # Pick a random instance
            chosen = random.choice(instances)
            print(f"\nCalling random instance: {chosen['address']}")
            try:
                resp = requests.get(chosen['address'])
                print(f"Response from {chosen['address']}: {resp.status_code} {resp.text}")
            except Exception as call_err:
                print(f"✗ Error calling instance: {call_err}")
        else:
